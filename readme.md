...
`zring` is an anchored bar for quick access to the "recent things" rings the OS already maintains internally. Today it exposes the MRU window ring as flat, one-click buttons docked to a screen edge — alongside, not replacing, the Windows Taskbar. A clipboard ring (Emacs kill-ring style multi-slot clipboard, click to paste at cursor) is planned next.

The name comes from those data structures: the *kill ring* and the *MRU window ring* that Alt+Tab traverses. Both already exist inside the OS — `zring` just makes them visible and one-click reachable.

Built as a .NET 10 WPF application; heavily uses the Win32 API and is therefore Windows-only by design (tested on Windows 11 and Windows 10).

`zring` is built on top of [AppSwitcherBar](https://github.com/adamecr/AppSwitcherBar) by Radek Adamec — the application-switching codebase is inherited from it. See [LICENSE](LICENSE) for the original copyright.

## Quick Start ##
To start `zring`, just run `zring.exe`. 

![Main zring window](doc/img/zring.png)
...
As mentioned above, the idea is not to use any invasive techniques. It's possible to get a good result when combining the `zring` with auto-hide functionality of standard Windows Taskbar. In such case the `zring` seems to be "main bar", although it's possible to simply get the Windows Taskbar by moving the mouse "over the outer edge".

`zring` periodically enumerates the available application windows and presents them as buttons in the bar. The current active (foreground) application window has the highlighted button. The button contains icon (if it can be retrieved) and the window title. The title can be cropped to fit the button size, the full title is available in tool-tip when hovering the mouse over the button.
...
The windows of the "same" applications don't group into single button (well, that's why I built the app), however the `zring` puts the application windows belonging to the same process together (that's what "grouping" is in context of `zring`).
...
`zring` shows also the buttons of the applications pinned to the task bar as transparent buttons with italics caption ("pins") when there is no window open for the application. The pin button doesn't have the thumbnail and click to the button launches the application.
...
Use the icon at the top to show/hide the button labels. The desktop icon toggles the desktop and the theme icon toggles between light and dark theme.Click on the power icon to close the `zring` application and restore the Windows content area.
...
The search results are sorted by a rank created from
 ...
 - count of runs and the last run date. This is updated when a application is launched from `zring`.
...
The run statistics used for building the search key are not persisted between `zring` runs.
...
*Note: Make sure the user running the `zring` has the read access to `Windows\Prefetch` directory when using the Prefetch data for search ranking (users don't have the access by default)!*
...
The collection of application windows (buttons) is periodically updated and the changes are applied incrementally to keep the order and position within the bar. Besides the data that can be directly retrieved during the window enumeration, `zring` also use some "global" helper data about applications installed and available for current user within the system. Installed applications data are used for filling some gaps when the proper information is not retrieved directly from application window, for example icon for Windows Store apps. Installed applications data are also important to get the right Application User Model ID (used for example for grouping or JumpLists) as only a few windows or processes provide it directly. Retrieving installed application data is an operation that typically takes a few seconds, so these helper data are being retrieved on background at startup. The main `zring` functionality is available during the background data retrieval, user is just notified by "Arrow with CD" cursor when hovering over the buttons, that the retrieval is still in progress.
...
It's possible to add `zring` to Windows Startup folder using the checkbox `Run on Win start`. This functionality can be disabled by settings (see the next chapter).
...
When the auto-size switch is on, the `zring` auto-size itself as well as the application window buttons. Buttons use the fixed height as defined in settings (`AppBarButtonHeight`), the width can vary - buttons can be make smaller up to ratio defined in settings (`AppBarButtonMinWidthRatio`) - default value is 50%.
...
The app filter (funnel) button lets you restrict the appbar to show only selected apps. Click the funnel icon to open a popup with a checkbox list of currently running apps (grouped by AppId, so multiple windows of the same app share a single entry). Tick the apps you want visible; untick to hide; untick everything (or press "Clear all") to restore the default "show everything" behavior. Selections persist in `appsettings.user.json` under `UserSettings.FilteredAppKeys` and are kept by AppId — closing and reopening an app keeps its filter state. Pinned apps are never affected by this filter. Enabled via `ShowAppFilter`.

*Note: The default communication device can't be set using the `zring` as it's not implemented. In general, the change of default device is utilizing undocumented API `IPolicyConfig`.*
...
```json
{
  "AppSettings": {
    ...
    "FeatureFlags": {
      ...
      "EnableRunInfoFromWindowsPrefetch":false,
...
- `ShowInTaskbar` - Flag whether to display the `zring` in the Taskbar (it automatically excludes itself from the list of windows in the bar regardless of this setting)
- `AllowRunOnWindowsStartup` - Flag whether to give use the configuration option to set `zring` to run on Windows startup
...
- `ShowPinnedApps` - Flag whether to show buttons ("pins") for applications pinned to taskbar when there is no window for pinned application. Default is `false`; when enabled together with the app filter, a pin button restores in place of any filtered-out window, which can visually defeat the filter.
...
- `CheckForIconChange` - Flag whether `zring` periodically checks windows for an icon change or whether the icon retrieved for the first time is used. Switching it of can give some performance gain, however some applications change their icon during their runtime to signal user some state information.
- `CheckForAppIdChange` - Flag whether `zring` periodically checks windows for change of their Application ID that has an impact to grouping the buttons together. It's off by default as the runtime changes of Application ID are quite rare.
- `InvertWhiteIcons` - When `zring` uses light WPF and the host Windows are set to dark theme, some icons provided by applications are in shadows of white (different opacity to be exact) that might be hard to see on the light background. When this option is on, `zring` tries to identify "all white" icons and invert the to black for better visual experience.
...
- `ShowAppFilter` - Flag whether to show the app filter (funnel) button in the app bar
...
- `FeatureFlags` - Section used to configure (and allow/block) the experimental features or work in progress.
  ...
  - `RunOnWindowsStartup` - Enable/Disable functionality manipulating the Windows startup link for `zring`. Use `AllowRunOnWindowsStartup` setting to hide the configuration option, use the feature flag to use the dummy implementation. This is probably gonna be used during the development only
  ...
  - `EnableRunInfoFromWindowsPrefetch` - When set, the Windows Prefetch files will be used to retrieve initial run statistics for search ranking. Default is `false`: `C:\Windows\Prefetch` is ACL'd to Administrators by default, so enabling this on a standard user account causes one `UnauthorizedAccessException` per installed app at startup (hundreds of log entries) and falls back to other sort keys anyway.
...
*Note: The application uses the standard [.NET Logging API](https://docs.microsoft.com/en-us/dotnet/core/extensions/logging) with [Serilog](https://serilog.net/) as the backing provider. Log files are written next to the executable under `logs\app-YYYYMMDD.log`, rolling daily with the last 14 files retained. Minimum level and per-namespace overrides are configured in the `Serilog` section of `appsettings.json`; see [Serilog.Settings.Configuration](https://github.com/serilog/serilog-settings-configuration) for the full schema.* 
...
Windows Taskbar provides the application window live thumbnails on mouse hover. This functionality is provided by Desktop Window Manage (DWM) and `zring` uses its API for the same functionality.
...
Application User Model ID (aka AUMID, AppId or Application ID) is an identifier used by Windows for grouping the buttons in taskbar. It's also key identifier of the modern applications (UWP/Windows Store apps). `zring` also use AppId for grouping as well as for other functionality like launching the applications from context menu, or retrieving the JumpLists.
...
When the setting `JumpListUseTempFiles` is set, `IPersistFile` interface of link is used to temporary store the link to file and read it as `IShellItem2` to get the access to shell properties providing the additional information about link that are used by `zring`.
...
As I didn't find any Windows API that will retrieve the list of applications pinned to Start, I checked the exports of the DLL backing the Power Shell module implementing `Export-StartLayout`. I found that it references and uses COM `IStartLayoutCmdlet` and corresponding class `CStartLayoutCmdlet`, so I embedded these into `zring` to get the same functionality without a need to host the Power Shell. The start menu layout is exported to temporary file using `IStartLayoutCmdlet`, the XML or JSON is then parsed and used as a source of information about pinned applications.
...
## Roadmap ##
  - **Clipboard ring** — keep the last N clipboard items in a visible ring on the bar; click an item to paste at the cursor position. Emacs `kill-ring` / Vim yank-register style multi-slot clipboard, surfaced as a UI element.
  - JumpLists for items in the application list
  - Optional auto-hide for the appbar


## Credits & Sources Used ##
 - `zring` is built on top of [AppSwitcherBar](https://github.com/adamecr/AppSwitcherBar) by Radek Adamec (MIT licensed) — the application-switching codebase is inherited from it.
 ...
 - [MahApps.Metro IconPacks](https://github.com/MahApps/MahApps.Metro.IconPacks) are good source of icons for WPF applications (was used in version 1 of the `zring`)
 - [WPF UI](https://github.com/lepoco/wpfui) is used since version 2 of `zring` for modern look and feel as well for suport of themes
