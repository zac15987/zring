...
namespace Zring.Win32.Services.Audio
{
    ...
    internal class AudioDevice : DisposableWithNotification
    {
        ...
        /// <summary>
        /// Serializes mutations of <see cref="AudioSessions"/> so WASAPI notification
        /// callbacks (deferred onto the ThreadPool) and <see cref="DoDispose"/> cannot race.
        /// </summary>
        private readonly object sessionsLock = new();

        /// <summary>
        /// 0 = idle, 1 = a refresh is queued on the ThreadPool.
        /// Used to coalesce bursts of WASAPI session notifications.
        /// </summary>
        ...
        public AudioDevice(IMMDevice device, AudioDirection direction)
        {
            ...
            if (hr.IsSuccess && result is IAudioSessionManager2 sessionManager)
            {
                ...
                {
                    // WASAPI forbids calling Un/RegisterAudioSessionNotification from inside
                    // an IAudioSessionNotification/IAudioSessionEvents callback. Defer the
                    // refresh onto the ThreadPool and coalesce bursts with refreshPending.
                    if (Interlocked.Exchange(ref refreshPending, 1) == 1) return;
        ...
        {
            // Unregister first so Windows stops delivering session events, then take the
            // lock to drain any refresh that the ThreadPool may already be running.
            Notify(false);
            lock (sessionsLock)
            {
                ClearSessions();
